...
def clean_database():
    ...
    # album
    album.models.Photo.objects.all().delete()
    # blog
...
def init_album():
	photo_csv_path = pathlib.Path(BASE_DIR).joinpath('scripts', 'initialize', 'album_photo.csv')
	csv_reader = csv.reader(open(photo_csv_path))
	for line in csv_reader:
		if line[2]:
			album.models.Photo.objects.create(foreignUrl=line[2])
