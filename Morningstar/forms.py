...
FAKE_PASSWORD = "********"
FAKE_PHONE = "19800000000"
FAKE_CODE = "100000"


class LoginForm(forms.Form):
    identity = forms.CharField(
        label="用户名/邮箱/手机号", initial="", required=True, widget=forms.TextInput(attrs={"class": "form-control"}))
    password_login = forms.CharField(
        label="密码", widget=forms.PasswordInput(attrs={"class": "form-control"},render_value=True), required=True)
...
class RegisterForm(forms.Form):
    ...
    phone = forms.CharField(label='手机号', required=True,
        widget=forms.TextInput(attrs={"placeholder": FAKE_PHONE}),
        validators=[RegexValidator(r'^(1[3|4|5|6|7|8|9])\d{9}$', '手机号格式错误'), ])

    code = forms.CharField(label='验证码',required=True,
        widget=forms.TextInput(attrs={"placeholder": FAKE_CODE}),
        validators=[RegexValidator(r'^\d{6}$', '验证码格式错误'),])
    ...
    def clean_code(self):
        ...
        conn = get_redis_connection("redis")
        redis_code = conn.get(f'{phone}-register')

        if not redis_code:
            self.add_error('code', '验证码失效或未发送，请重新发送')

        if code.strip() != str(redis_code.decode()):
