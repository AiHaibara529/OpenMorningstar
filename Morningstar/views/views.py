...
from ..forms import LoginForm, RegisterForm, SendSmsForm, FAKE_EMAIL, FAKE_PHONE
        ...
        if request.method == "POST":
            ...
            if "confirm_password" in request.POST:
                ...
                if form.is_valid():
                    ...
                    # 判断为手机号注册
                    elif email == FAKE_EMAIL:                        
                        user = User.objects.create(
                            username=username, password=make_password(password), phone=phone, is_active=True)

                        auth.login(request, user)
                        response_data = {'status': 'success', 'message': '成功通过手机号注册'}
                        return JsonResponse(response_data)
                    else:
                        # 后续添加Oauth2
                        pass
